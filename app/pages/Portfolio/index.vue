<template>
	<div
		class="flex flex-col gap-5 bg-[var(--p-primary-background-color)] rounded-2xl backdrop-blur-sm w-full h-[80%] mt-50 p-10 mr-10 ml-10 overflow-scroll">
		<div v-if="portfolioPages" class="grid grid-cols-4 gap-20">
			<div
				v-for="(page, key) in portfolioPages"
				:key="key"
				class="portfolio-card flex flex-col gap-5 justify-center items-center bg-[#0e1116e7] backdrop-blur-sm p-10 rounded-2xl h-[50vh]">
				...
				<div class="grid grid-cols-4 gap-2 overflow-x-scroll rounded-2xl h-[25%]">
				...
				<div class="flex flex-row gap-5">
					<Button
						...
						" />
					<Button icon="pi pi-eye" label="Lire plus" @click.stop="navigateTo(`/portfolio${page.path}`)" />
				</div>
			</div>
		</div>
	</div>
...
<script lang="ts" setup>
	...
	onMounted(async () => {
		...
		if (pages.value) {
			pages.value = pages.value.sort((p1, p2) => {
		...
		portfolioPages.value = pages.value;
	});
</script>
