...
export class MainMenuScene extends RScene {
	...
	private readonly cameraLerpDuration = 500; // durée en ms (1.5s)
	...
	async Init() {
		...
		this.sunLight = new THREE.DirectionalLight(lightColor, lightIntensity);
	...
	animate() {
		...
		if (this.isRotatingToCoordinate && this.targetRotationQuaternion && this.planet && this.clouds) {
		...
		if (
			...
			this.getCamera().quaternion.slerpQuaternions(this.startCameraQuaternion, this.targetCameraQuaternion, t);
	...
	public rotatePlanetTo(position: THREE.Vector3) {
		...
		const direction = new THREE.Vector3().subVectors(position, this.planet.position).normalize();
	...
	public rotatePlanetToQt(targetQuaternion: THREE.Quaternion, speed: number = 0.05) {
		if (!this.planet) return;
		if (!this.clouds) return;
	...
	public pointCameraTo(position: THREE.Vector3) {
		...
		const direction = new THREE.Vector3().subVectors(position, this.getCamera().position).normalize();
	...
	public pointCameraToQt(targetQuaternion: THREE.Quaternion, speed: number = 0.05) {
		if (!targetQuaternion) return;
		this.isResettingCameraPosition = false;

		this.getCamera().quaternion.slerp(targetQuaternion, speed);

		const angleDifference = this.getCamera().quaternion.angleTo(targetQuaternion);

		if (Number.isNaN(angleDifference)) return;
